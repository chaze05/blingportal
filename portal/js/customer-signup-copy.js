...
var function_calls = [do_nothing,displayBlingNumber, display_welcome, display_voicemail,display_connectcall, do_nothing, do_nothing]
...
$(document).ready(function () {
    ...
    $('#selectplan').show();
    $('#welcome').hide();
//    $('#nexttitle')[0].innerText = "Lets Move to"
    ...
    console.log("Next screen")
        switch(current_screen_index) {
            ...
            case 4: display_next_screen();
                break;
            case 5: display_next_screen();
                break;
            case 6: display_next_screen();
                break;
        }
}
...
function displayBlingNumber() {
    ...
    if (signup_details['blingphonenumber'] == '') {
        ...
        $('#currentnum').show();
        $('#currentblingnumber')[0].innerText = signup_details['blingphonenumber']
        $('#nextbutton').show();
        $('#nexttitle')[0].innerText = "Like the current number. Lets move to next step."
    }
}

function display_welcome() {
    $('#currentwelcome')[0].value = signup_details['welcome_message']
    $('#nexttitle')[0].innerText = "Like the current message. Lets move to next step."
}
...
function display_connectcall() {
    $('#nexttitle')[0].innerText = "Nothing to discuss. Good to hear. Lets move to next step."
}
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for(i=0; i< data.length; i++) {
                ...
                div.innerHTML = "<Button class='btn button-next' id="+data[i]+" onclick=addblingnumber(this.id)>" + data[i] + "</Button>"
...
function requestcallback() {
    ...
    $.post("/slack/note", {message: content, channel: "https://hooks.slack.com/services/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"}, function(data){
                console.log(data)
            })
}
